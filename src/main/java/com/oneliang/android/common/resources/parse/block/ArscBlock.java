...
public class ArscBlock extends ComplexBlock{
...

	{
		...
		this.blockWrapperQueue.add(new BlockWrapper(ID_PACKAGE_COUNT,packageCountBlock));//header,package count,header count 12byte
		this.blockWrapperQueue.add(new BlockWrapper(ID_STRING_POOL,this.globalStringPoolBlock));//resource string pool,style content
	}
	...
	public void print(){
		...
		if(this.tablePackageBlockList!=null){
			for(TablePackageBlock tablePackageBlock:this.tablePackageBlockList){
				...
				Logger.log("----------");
//				TreeSet<ResourceEntry> allResourceTreeSet=new TreeSet<ResourceEntry>(tablePackageBlock.tablePackage.allResourceEntryList);
...
//		List<String> resourceTypeStringList=this.resourceTypeStringPoolBlock.stringPool.stringList;
//		for(String resourceType:resourceTypeStringList){
//			Logger.log("resource type:"+resourceType);
//		}
//		List<String> keywordStringList=this.resourceKeywordStringPoolBlock.stringPool.stringList;
//		Logger.log("keyword string list size:"+keywordStringList.size());
//		for(TableTypeSpecBlock tableTypeSpecBlock:TableTypeSpecBlock.tableTypeSpecBlockList){
//			for(TableType tableType:tableTypeSpecBlock.tableTypeSpec.tableTypeList){
//				int i=0;
//				for(Entry entry:tableType.entryList){
//					Logger.log(resourceTypeStringList.get(tableType.resourceTypeId-1)+" "+keywordStringList.get(entry.index)+" "+StringUtil.byteToHexString(MathUtil.intToByteArray(0x7F000000|(tableType.resourceTypeId<<0x10)|tableType.entryOrderList.get(i))));
//					i++;
//				}
//			}
//		}
...
//			TableTypeSpecBlock tableTypeSpecBlock=TableTypeSpecBlock.tableTypeSpecBlockList.get(typeId-1);
//			Entry entry=tableTypeSpecBlock.tableTypeSpec.tableTypeList.get(0).entryList.get(order);
...
//			short order=MathUtil.byteArrayToShort(new byte[]{intByteArray[2],intByteArray[3]});
////			TableTypeSpecBlock tableTypeSpecBlock=TableTypeSpecBlock.tableTypeSpecBlockList.get(typeId-1);
//			Entry entry=null;//tableTypeSpecBlock.tableTypeSpec.tableTypeList.get(0).entryList.get(order);
////			entry.value
//			int offset=this.headerBlock.getTotalSize()+4;
//			int stringOffset=this.globalStringPoolBlock.stringPool.stringOffsetArray[entry.value];
...
//					nextStringOffset=this.globalStringPoolBlock.stringPool.header.chunkSize-this.globalStringPoolBlock.stringPool.stringCount*4-28-3;//28 is string pool header size,3 is string pool end
//				}else{
