...
public class TablePackageBlock extends ComplexBlock {
	...
	private static final int GENERAL_BLOCK_SIZE=284;//some time 288 byte,so must be check
...
//	private TableTypeSpecBlock tableTypeSpecBlock=null;
	...
	{
		this.blockWrapperQueue.add(new BlockWrapper(ID_HEADER,this.headerBlock));//package table,type
		this.blockWrapperQueue.add(new BlockWrapper(ID_PACKAGE_ID,this.packageIdBlock));//package table,package id
		this.blockWrapperQueue.add(new BlockWrapper(ID_PACKAGE_NAME,this.packageNameBlock));//package table,package name
		this.blockWrapperQueue.add(new BlockWrapper(ID_RESOURCE_TYPE_OFFSET,this.resourceTypeOffetBlock));//package table,resource type offset
		this.blockWrapperQueue.add(new BlockWrapper(ID_LAST_PUBLIC_TYPE,this.lastPublicTypeBlock));//package table,last public type
		this.blockWrapperQueue.add(new BlockWrapper(ID_RESOURCE_KEYWORD_OFFSET,this.resourceKeywordOffsetBlock));//package table,resource key word offset
		this.blockWrapperQueue.add(new BlockWrapper(ID_LAST_PUBLIC_KEY,this.lastPublicKeyBlock));//package table,last public key,header count 284 byte
