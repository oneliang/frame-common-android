...
public class HeaderBlock extends ComplexBlock {
	...
	private Queue<BlockWrapper> blockWrapperQueue=new ConcurrentLinkedQueue<BlockWrapper>();
	{
		...
		this.blockWrapperQueue.add(new BlockWrapper(ID_HEADER_SIZE,new UnitBlock(Endian.LITTLE,2)));//header,header size
		this.blockWrapperQueue.add(new BlockWrapper(ID_BLOCK_SIZE,new UnitBlock(Endian.LITTLE,4)));//header,file size
	}
